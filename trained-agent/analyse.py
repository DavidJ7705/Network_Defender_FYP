...
def plot_overlay_with_mean(dfs: dict, out: Path, window: int = 20):
    ...
    ax.set_xlabel("Episode", fontsize=11); ax.set_ylabel("Average Reward", fontsize=11)
...
def plot_per_episode_detail(dfs: dict, out: Path):
    """
    NEW — Per-episode scatter + rolling MA + trend line + worst-episode marker.
    One panel per red agent.
    """
    ...
    for ax, agent in zip(axes, agents):
        ...
        ax.plot(eps, np.poly1d(z)(eps), "--", color=col, alpha=0.7, linewidth=1.5, label="Trend")
    ...
    title(fig, "Per-Episode Reward Detail — CC4 Blue Agent Evaluation")
    fig.tight_layout()
...
def plot_action_distribution(action_counters: dict, out: Path):
    """
    NEW — Heatmap + stacked bar of blue-agent action-type frequencies.
    """
    ...
    for i, (action, color) in enumerate(zip(action_types, bar_colors)):
        ...
        bottoms += vals
    ax2.set_xticks(x); ax2.set_xticklabels(agent_labels, color="white", fontsize=9)
    ...
    title(fig, "Blue Agent Action Analysis — What is the Agent Actually Doing?")
    fig.tight_layout()
...
def plot_stats_table_and_boxplot(dfs: dict, metrics: dict, out: Path):
    """
    NEW — Box plot + comprehensive stats table side by side.
    """
    ...
    cell_data  = []
    for key, fmt in [("mean","{:.1f}"),("std","{:.1f}"),("median","{:.1f}"),
                     ("min","{}"),("max","{}"),("iqr","{:.1f}"),
                     ("cv","{:.1f}%"),("below_300","{}"),("below_500","{}")]:
        row = [next(l for l in row_labels if l.lower().startswith(key[:3].lower()))]
        for a in agents_present:
            row.append(fmt.format(metrics[a][key]))
        cell_data.append(row)
    # fix row label mismatch — just use direct labels
    ...
    for (r, c), cell in tbl.get_celld().items():
        cell.set_edgecolor("#333"); cell.set_linewidth(0.5)
        if r == 0:
            cell.set_facecolor(hdr_bg[c] if c < len(hdr_bg) else "#16213e")
            cell.set_text_props(color="white", fontweight="bold")
        else:
            shades = ["#1a1a2e","#1b2e1b","#2e1b1b","#2e2010"]
            cell.set_facecolor(shades[c] if c < len(shades) else "#1a1a2e")
            agent_colors = [TEXT_COL, "#2ecc71", "#e74c3c", "#f39c12"]
            cell.set_text_props(color=agent_colors[c] if c < len(agent_colors) else TEXT_COL)

    ax2.set_title("Comprehensive Statistical Summary", color="white",
                  fontsize=11, fontweight="bold", pad=15)

    title(fig, "Statistical Analysis — CybOrg Cage 4 Blue Agent vs Red Agent Types")
    fig.tight_layout()
...
def plot_failure_analysis(dfs: dict, out: Path):
    """
    NEW — Histogram with catastrophic zone + cumulative % above threshold.
    """
    fig, (ax1, ax2) = styled_fig(1, 2, (16, 5))
    agents_present = [a for a in RED_AGENTS if a in dfs]
    ...
    for agent in agents_present:
        ax1.hist(dfs[agent]["reward"].values, bins=bins, alpha=0.55,
                 color=COLORS[agent], label=LABELS[agent], edgecolor="black", linewidth=0.4)
    ax1.axvline(-300, color="white", linewidth=1.5, linestyle="--", alpha=0.7, label="–300 threshold")
    ax1.axvline(-500, color="red",   linewidth=1.5, linestyle="--", alpha=0.9, label="Catastrophic –500")
    ax1.axvspan(-650, -500, alpha=0.12, color="red")
    ax1.text(-575, ax1.get_ylim()[1] * 0.85 if ax1.get_ylim()[1] > 1 else 3,
             "Catastrophic\nZone", ha="center", color="red", fontsize=8)
    style_ax(ax1)
    ax1.set_title("Episode Reward Histogram", color="white", fontsize=11, fontweight="bold")
    ax1.set_xlabel("Episode Reward"); ax1.set_ylabel("Frequency")
    ax1.legend(facecolor=BG_PANEL, labelcolor="white", fontsize=8)

    # Cumulative % above threshold
    ...
    ax2.text(-300, 5, " –300", color="white", fontsize=8)
    ax2.text(-500, 5, " –500", color="red",   fontsize=8)
    ax2.set_ylim(0, 105)
    style_ax(ax2)
    ax2.set_title("Cumulative % of Episodes Above Reward Threshold",
                  color="white", fontsize=11, fontweight="bold")
    ax2.set_xlabel("Reward Threshold"); ax2.set_ylabel("% of Episodes Above Threshold")
    ax2.legend(facecolor=BG_PANEL, labelcolor="white", fontsize=8)

    title(fig, "Failure Mode Analysis — Catastrophic Episode Investigation")
    fig.tight_layout()
...
def plot_learning_stability(dfs: dict, out: Path):
    """
    NEW — Rolling volatility + first-half vs second-half mean comparison.
    """
    fig, (ax1, ax2) = styled_fig(1, 2, (16, 5))
    agents_present = [a for a in RED_AGENTS if a in dfs]
    window = 10
    ...
    for agent in agents_present:
        ...
        ax1.plot(roll_std, color=COLORS[agent], linewidth=2, label=LABELS[agent], alpha=0.9)
    style_ax(ax1)
    ax1.set_title(f"Rolling Volatility (Std Dev, window={window})",
                  color="white", fontsize=11, fontweight="bold")
    ax1.set_xlabel("Episode"); ax1.set_ylabel("Rolling Std Dev")
    ...
    def make_darker(hex_col, factor=0.5):
    ...
    title(fig, "Learning Stability & Temporal Performance Analysis")
    fig.tight_layout()
